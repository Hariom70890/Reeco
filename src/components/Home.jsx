   ...
   const handleEditSubmit = async () => {
      try {
         if (editedProduct.price >= 0 && editedProduct.qty >= 0) {
            ...
            await fetch(`${url}/products/${selectedProductId}`, {
               method: "PUT",
            ...
            <tbody>
               {products?.map((product) => (
                  <tr key={product.id}>
                     ...
                     <TableTd>
                        <Tabledata status={product.status}>
                           <TextForStatus style={{ color: "white" }}>
                              {product.status}
                           </TextForStatus>{" "}
                        </Tabledata>
                     </TableTd>
                     <TableTd>
                        <Button
                           onClick={() => handleYesClick(product.id)}
                           color={
                     ...
                     <TableTd>
                        <Button
                           onClick={() => handleNoClick(product.id)}
                           color={product.status == "Missing" ? "red" : "green"}
                        >
                           <RxCross1 />
                        </Button>
                     </TableTd>
